...
class docs_and_tech(commands.Cog):
    ...
    async def help(
        ...
        emb.add_field(
            name='Экономика', 
            value='''`!работа` `!премиум` `!баланс` `!перевод` `!депозит` `!снять` 
`!кено` `!магазин` `!купить` `!роль+` `!роль-` `!крипто` 
`!крипто_купить` `!крипто_продать`''',
            inline=False
            )
        emb.add_field(
            name='Утилиты',
            value='`!центр` `!ответы` `!ошибки` `!упоминание`',
            inline=False
            )
        emb.add_field(
            name='Фан',
            value='`!аватар` `!число` `!монетка` `!пёсик` `!кот`',
            inline=False
            )
#        emb.add_field(
...
{voice} Голосовых: **{len(ctx.guild.voice_channels)}**''', inline=False)
    ...
    async def profile(
        ...
        if member.activity == None or member.activity.name == None:
            emb.add_field( # 1 3
                name="Пользовательский статус/Текст отсутствует",
                value="Не доступен",
            )
        else:
            emb.add_field( # 1 3
...
*Поистине могучий)*.''',
            ...
            pass
        if eco["users"]["money"][str(member.id)] + eco["users"]["bank"][str(member.id)] >= 1000000000:
            emb2.add_field(
                name=f':coin: | Миллиардер',
                value='''╰ Имеет на карманном балансе и в банке в сумме больше 1.000.000.000₽!''',
                inline=False
            )
        else:
            pass
        if eco["users"]["money"][str(member.id)] + eco["users"]["bank"][str(member.id)] >= 1000000:
            emb2.add_field(
                name=f':coin: | Миллионер',
                value='''╰ Имеет в сумме больше 1.000.000₽.''',
                inline=False
            )
        else:
            pass
    ...
    async def bot(
        ...
        embed.add_field(
            name='Поддержка',
            value=':green_circle: [ТехПоддержка](https://discord.gg/bF4SGguHxC)'
        )
        embed.add_field(
            name="Команд", 
            value=f'> 34'
        ...
        for guild in self.Bot.guilds:
            if guild.shard_id == ctx.guild.shard_id:
                for member in guild.members:
                    members += 1
        embed.add_field(
            name=f"Кластер {shard_id} из {len(self.Bot.shards)}",
            value=f"> `Пинг кластера:` {shard_ping}мс\n> `Пользователей на кластере:` {members}\n> `Серверов на кластере:` {shard_servers}",
        )
        owner = self.Bot.get_user(599490357754855435)
        embed.set_footer(
            text=f"By {owner}", 
...
Страница 3 из 7''')
        emb4 = discord.Embed(
            ...
            description='''**!магазин**: `Нет аргументов`
    Показывает список ролей из магазина сервера.
**!купить**: `!купить <@роль>`
    Позволяет купить роль из магазина сервера.
**!роль+**: `!роль+ <@роль> <цена> [тип лота] [тип скидки]`
    Позволяет выставить указанную роль в магазин сервера.
**!роль-**: `!роль- <@роль>`
    Позволяет убрать указанную роль из магазина сервера.
...
**!число**: `!число <число 1> <число 2>`
    Бот выведет рандомное число из заданного Вами промежутка.
...
Страница 6 из 7''')
        emb7 = discord.Embed(
            ...
            description='''Хотите предложить собственные команды в бота?
Присоединяйтесь к [официальному серверу бота](https://discord.gg/dVbPMUKFnh) и предложите свою идею для улучшения проекта.''',
            color=base_color
            )
        emb7.set_footer(text='Страница 7 из 7')
